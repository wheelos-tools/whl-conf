...
def handle_remove(manager: ConfigManager, args: argparse.Namespace):
    ...
    logging.info(
        f"Attempting to remove paths from active config: {args.paths_to_remove}"
    )
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser_create.add_argument("template_name", nargs="?", help="Template config name")
    parser_create.add_argument("new_config_name", help="New config name")
    # 4. delete
    parser_delete = subparsers.add_parser("delete", help="Delete the specified config")
...
def main():
    ...
    try:
        ...
        command_handlers: Dict[
            str, Callable[[ConfigManager, argparse.Namespace], None]
        ] = {
            "list": handle_list,
            "info": handle_info,
    ...
    # Precise, user-facing exception handling
        ...
        logging.error(f"Operation failed: Specified config not found. Details: {e}")
        ...
        logging.error(f"File system error: File or directory not found. Details: {e}")
        sys.exit(5)
