...
class ConfigManager:
    ...
    def create_config(self, template_name: str, new_config_name: str):
        ...
        try:
            ...
            if template_name:
                ...
                shutil.copytree(template_path, new_config_path, dirs_exist_ok=True)
                ...
                try:
                    template_content = read_resource_content(
                        "whl_conf.data", "default_template.yaml"
                    )
                    # Use a more descriptive name for the list of items
                    template_items = template_content.splitlines()
                except (ModuleNotFoundError, FileNotFoundError) as e:
                    raise RuntimeError(
                        "Could not locate the default template data files package 'whl_conf.data'."
                    ) from e
            ...
            meta_file = self._get_meta_manager_for(new_config_name).meta_path
    ...
    def pull_config(self, config_name: str) -> None:
        ...
        base_url = os.environ.get(
            "WHEELOS_RESOURCE_URL",
            "https://github.com/wheelos/vehicles-config/releases/download/v1.0.0",
        )
        ...
        with tempfile.TemporaryDirectory(suffix="_conf_pull") as temp_dir:
            ...
            try:
                ...
                raise ConfigError(
                    f"Network error while downloading from {url}: {e}"
                ) from e
            except Exception as e:
            ...
            try:
                ...
                raise ConfigError(
                    f"Failed to move unpacked configuration to final destination: {e}"
                ) from e
    ...
    def add_active_config(self, source_paths: List[str], dry_run: bool = False):
        ...
        for source_file in all_source_files:
            try:
                ...
                raise ValueError(
                    f"Source path '{source_file}' must be inside the base directory '{self.base_dir}'."
                )

            if relative_path in current_manifest_paths:
                logging.info(
                    f"Path '{relative_path}' is already in the manifest. Skipping."
                )
                continue
        ...
        if not actions_to_perform:
            print(
                "No new files to add. All specified paths are already in the manifest."
            )
            return
        ...
        try:
            ...
            print(
                f"Successfully added files to active configuration '{active_config_name}'."
            )
    ...
    def remove_active_config(self, paths_to_remove: List[str], dry_run: bool = False):
        ...
        if not active_config_name:
            raise ConfigActiveError(
                "Cannot remove: No configuration is currently active."
            )
        ...
        if dry_run:
            print(
                f"--- Dry Run: Removing paths from active config '{active_config_name}' ---"
            )
        ...
        if dry_run:
            print(
                f"The following {len(manifest_entries_to_delete)} items would be removed:"
            )
        ...
        # --- Phase 2: Execution ---
        logging.info(
            f"Removing {len(manifest_entries_to_delete)} item(s) from '{active_config_name}'."
        )
        ...
        try:
            ...
            for rel_path in reversed(sorted(list(manifest_entries_to_delete))):
                ...
                if copied_file_path.is_file():
                    ...
                    try:
                        ...
                        while parent != active_config_path and not any(
                            parent.iterdir()
                        ):
                            parent.rmdir()
            ...
            logging.critical(
                f"A critical error occurred during removal: {e}. The manifest may be out of sync.",
                exc_info=True,
            )
            raise ConfigError(
