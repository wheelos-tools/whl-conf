...
# --- ConfsLock Class (Adjusted for Python 3.9 Type Hinting) ---
...
class ConfsLock:
    ...
    def __enter__(self):
        ...
        try:
            ...
            raise LockError(f"Failed to create lock directory {self.directory_to_lock}: {e}") from e
        ...
        try:
            ...
            self.logger.error(f"An unexpected error occurred while acquiring lock: {e}", exc_info=True)
            raise LockError(f"Failed to acquire lock due to an unexpected error: {e}") from e
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            # os.path.exists is fully compatible
...
def attribute_lock(attr_name: str, timeout: float = 10.0) -> Callable[[Callable[..., T]], Callable[..., T]]:
    ...
    """
    # Python 3.9 Compatibility: Use Callable[..., T] for the wrapped function.
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        @functools.wraps(func)
        def wrapper(*args: Any, **kwargs: Any) -> T:
            # First argument is always the instance `self` for instance methods
            if not args:
                raise TypeError(f"Locking decorator expects '{func.__name__}' to be a bound method.")
            ...
            if not isinstance(lock_dir, Path):
                raise TypeError(f"Attribute '{attr_name}' must be a Path object, but got {type(lock_dir)}.")
            ...
            try:
                ...
                raise LockError(f"A lock error occurred during operation '{func.__name__}': {e}") from e
        return wrapper
    return decorator
...
def method_call_lock(method_name: str, param_name: str, timeout: float = 10.0) -> Callable[[Callable[..., T]], Callable[..., T]]:
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            if not isinstance(lock_dir, Path):
                raise TypeError(f"Method '{method_name}' must return a Path object, but got {type(lock_dir)}.")
